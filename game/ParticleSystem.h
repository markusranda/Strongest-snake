...
struct ParticleSystem
{
    ...
    VkDescriptorSetLayout computeSpawnDescriptorSetLayout = VK_NULL_HANDLE;
    VkDescriptorSetLayout computeCountersDescriptorSetLayout = VK_NULL_HANDLE;
    
    VkDescriptorSet computeSimDescriptorSets[2];
    VkDescriptorSet computeSpawnDescriptorSets[2];
    VkDescriptorSet computeCountersDescriptorSet = VK_NULL_HANDLE;
    
    VkDescriptorPool computeSimDescriptorPool = VK_NULL_HANDLE;
    VkDescriptorPool computeSpawnDescriptorPool = VK_NULL_HANDLE;
    VkDescriptorPool computeCountersDescriptorPool = VK_NULL_HANDLE;
    
    VkPipelineLayout computeSimPipelineLayout = VK_NULL_HANDLE;
    VkPipelineLayout computeSpawnPipelineLayout = VK_NULL_HANDLE;
    VkPipelineLayout computeCountersPipelineLayout = VK_NULL_HANDLE;
    
    VkPipeline computeSimPipeline = VK_NULL_HANDLE;
    VkPipeline computeSpawnPipeline = VK_NULL_HANDLE;
    VkPipeline computeCountersPipeline = VK_NULL_HANDLE;

    // Graphics pipeline
    VkDescriptorSetLayout graphicsDescriptorSetLayout = VK_NULL_HANDLE;
    VkDescriptorSet graphicsDescriptorSets[2];
    ...
    void createComputeSimPipeline(VkDevice &device)
    {
        ...
        if (vkCreatePipelineLayout(device, &pipelineLayout, nullptr, &computeSimPipelineLayout) != VK_SUCCESS) {
        ...
        if (vkCreateDescriptorPool(device, &poolInfo, nullptr, &computeSimDescriptorPool) != VK_SUCCESS)
        {
        ...
        VkDescriptorSetAllocateInfo allocInfo = {
            ...
            .descriptorPool = computeSimDescriptorPool,
            .descriptorSetCount = 2,
            .pSetLayouts = setLayouts,
        };
        if (vkAllocateDescriptorSets(device, &allocInfo, computeSimDescriptorSets) != VK_SUCCESS) {
        ...
        for (size_t i = 0; i < 2; i++) {
            ...
            writes[i][0] = {
                ...
                .dstSet = computeSimDescriptorSets[i],
                .dstBinding = 0,
                .descriptorCount = 1,
                .descriptorType = VK_DESCRIPTOR_TYPE_STORAGE_BUFFER,
        ...
        computeSimPipeline = CreateComputePipeline(device, "shaders/comp_particle_sim.spv", computeSimPipelineLayout);
    }

    void createComputeSpawnPipeline(VkDevice &device) {
        ...
        CreateDescriptorSetLayout(device, bindings, bindingsLen, computeSpawnDescriptorSetLayout);
        ...
        VkPipelineLayoutCreateInfo pl = {
            .sType = VK_STRUCTURE_TYPE_PIPELINE_LAYOUT_CREATE_INFO,
            .setLayoutCount = 1,
            .pSetLayouts = &computeSpawnDescriptorSetLayout,
            .pushConstantRangeCount = 1,
            .pPushConstantRanges = &pushRange,
        };
        if (vkCreatePipelineLayout(device, &pl, nullptr, &computeSpawnPipelineLayout) != VK_SUCCESS) {
        ...
        if (vkCreateDescriptorPool(device, &poolInfo, nullptr, &computeSpawnDescriptorPool) != VK_SUCCESS) {
            throw std::runtime_error("failed to create spawn descriptor pool");
        }

        // Allocate descriptor set
        VkDescriptorSetLayout setLayouts[2] = { computeSpawnDescriptorSetLayout, computeSpawnDescriptorSetLayout };
        VkDescriptorSetAllocateInfo allocInfo = {
            .sType = VK_STRUCTURE_TYPE_DESCRIPTOR_SET_ALLOCATE_INFO,
            .descriptorPool = computeSpawnDescriptorPool,
            .descriptorSetCount = 2,
            .pSetLayouts = setLayouts,
        };
        if (vkAllocateDescriptorSets(device, &allocInfo, computeSpawnDescriptorSets) != VK_SUCCESS) {
        ...
        for (uint32_t i = 0; i < 2; i++) {
            ...
            writes[i][0] = {
                .sType = VK_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET,
                .dstSet = computeSpawnDescriptorSets[i],               
                .dstBinding = 0,
                .descriptorCount = 1,
                .descriptorType = VK_DESCRIPTOR_TYPE_STORAGE_BUFFER,
        ...
        computeSpawnPipeline = CreateComputePipeline(device, "shaders/comp_particle_spawn.spv", computeSpawnPipelineLayout);
    }

    void createComputeCountersipeline(VkDevice &device) {
        ...
        CreateDescriptorSetLayout(device, bindings, bindingsLen, computeCountersDescriptorSetLayout);

        // Pipeline layout: 1 set, no push constants
        VkPipelineLayoutCreateInfo pipelineLayout = {
            .sType = VK_STRUCTURE_TYPE_PIPELINE_LAYOUT_CREATE_INFO,
            .setLayoutCount = 1,
            .pSetLayouts = &computeCountersDescriptorSetLayout,
            .pushConstantRangeCount = 0,
            .pPushConstantRanges = nullptr,
        };

        if (vkCreatePipelineLayout(device, &pipelineLayout, nullptr, &computeCountersPipelineLayout) != VK_SUCCESS) {
        ...
        if (vkCreateDescriptorPool(device, &poolInfo, nullptr, &computeCountersDescriptorPool) != VK_SUCCESS) {
        ...
        VkDescriptorSetAllocateInfo allocInfo = {
            .sType = VK_STRUCTURE_TYPE_DESCRIPTOR_SET_ALLOCATE_INFO,
            .descriptorPool = computeCountersDescriptorPool,
            .descriptorSetCount = 1,
            .pSetLayouts = &computeCountersDescriptorSetLayout,
        };
        if (vkAllocateDescriptorSets(device, &allocInfo, &computeCountersDescriptorSet) != VK_SUCCESS) {
        ...
        {
            .sType = VK_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET,
            .dstSet = computeCountersDescriptorSet,
            .dstBinding = 0,
            .descriptorCount = 1,
            .descriptorType = VK_DESCRIPTOR_TYPE_STORAGE_BUFFER,
            .pBufferInfo = &countersInfo,
        ...
        computeCountersPipeline = CreateComputePipeline(device, "shaders/comp_particle_counters.spv", computeCountersPipelineLayout);
    }
    ...
    void createGraphicsPipeline(RendererApplication &app, RendererSwapchain &swapchain)
    {
        ...
        CreateDescriptorSetLayout(device, &particleBinding, 1, graphicsDescriptorSetLayout);
        ...
        for (uint32_t i = 0; i < 2; i++) 
        {
            writes[i] = {
                .sType = VK_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET,
                .dstSet = graphicsDescriptorSets[i], 
                .dstBinding = 0,
                .descriptorCount = 1,
                .descriptorType = VK_DESCRIPTOR_TYPE_STORAGE_BUFFER,
    ...
    void init(RendererApplication &app, RendererSwapchain &swapchain) {
        ...
        CreateBuffer(device, app.physicalDevice, spawnSize, usage, properties, spawnBuffer, spawnMemory);
        CreateBuffer(device, app.physicalDevice, countersSize, usage, properties, countersBuffer, countersMemory);
        ...
        vkMapMemory(device, countersMemory, 0, countersSize, 0, &countersMapped);
        {
        ...
        // Pipelines
        createComputeSimPipeline(device);
        createComputeCountersipeline(device);
        createComputeSpawnPipeline(device);
    ...
    void recordSimCmds(VkCommandBuffer &cmd, float delta) {
        ...
        // PASS ONE (SIMULATE):
        // ====================================
        vkCmdBindPipeline(cmd, VK_PIPELINE_BIND_POINT_COMPUTE, computeSimPipeline);
        vkCmdBindDescriptorSets(cmd, VK_PIPELINE_BIND_POINT_COMPUTE, computeSimPipelineLayout, 0, 1, &computeSimDescriptorSets[descriptorSetSimIndex], 0, nullptr);
        vkCmdPushConstants(cmd, computeSimPipelineLayout, VK_SHADER_STAGE_COMPUTE_BIT, 0, sizeof(float), &simDelta);
        ...
        // PASS TWO (SPAWN):
        // ====================================
        vkCmdBindPipeline(cmd, VK_PIPELINE_BIND_POINT_COMPUTE, computeSpawnPipeline);
        vkCmdBindDescriptorSets(cmd, VK_PIPELINE_BIND_POINT_COMPUTE, computeSpawnPipelineLayout, 0, 1, &computeSpawnDescriptorSets[descriptorSetSimIndex], 0, nullptr);
        vkCmdPushConstants(cmd, computeSpawnPipelineLayout, VK_SHADER_STAGE_COMPUTE_BIT, 0, sizeof(uint32_t), &frameseed);
        ...
        // PASS TWO (SPAWN):
        // ====================================
        vkCmdBindPipeline(cmd, VK_PIPELINE_BIND_POINT_COMPUTE, computeCountersPipeline);
        vkCmdBindDescriptorSets(cmd, VK_PIPELINE_BIND_POINT_COMPUTE, computeCountersPipelineLayout, 0, 1, &computeCountersDescriptorSet, 0, nullptr);
    ...
    void recordDrawCmds(VkCommandBuffer &cmd, Camera &camera) {
        ...
        vkCmdBindDescriptorSets(cmd, VK_PIPELINE_BIND_POINT_GRAPHICS, graphicsPipeline.layout, 0, 1, &graphicsDescriptorSets[drawIdx], 0, nullptr);
